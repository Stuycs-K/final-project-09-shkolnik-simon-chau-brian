...
#encoding
text = open(TEXT_FILE_NAME, "rb").read().strip()
bin_data = string_to_bin(bytes(chr(len(text)), "utf-8") + text)
...
for i_frame in range(len(bin_data)):
  ...
  if max(ONES[i_frame], ZEROS[i_frame]) > 1: #TODO less arbitrary threshold
    SKIP_FRAME = True
    freqs = [FREQ_0, FREQ_1]
  elif v == "1":
    freqs = [FREQ_1]
